...
public class 修改社团信息 extends JFrame {
	...
	public 修改社团信息(String s) {
		...
		JLabel 修改成功 = new JLabel("\u4FEE\u6539\u6210\u529F\uFF01\u8BF7\u8FD4\u56DE\uFF01");
		修改成功.setVisible(false);
		...
		确认.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				CallableStatement cstmt = dbConn.prepareCall("{call dbo.EditAssociation(?,?,?,?)}");
				...
				cstmt.executeUpdate();//executeUpdate()用于添加删除等,executeQuery()用于产生单个结果集的语句
				stmt.close(); 
			}
				catch(Exception ea){
				ea.printStackTrace();
		...
		修改成功.setFont(new Font("宋体", Font.PLAIN, 15));
		修改成功.setBackground(Color.BLACK);
		修改成功.setForeground(Color.BLUE);
		修改成功.setBounds(288, 371, 136, 23);
		contentPane.add(修改成功);
